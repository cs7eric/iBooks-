            ...
            <div class="container-fluid" style="margin-bottom: 50px;color: var(--bs-gray);background: #1f2022;width: 1100px;">
                        ...
                        <div class="col-12">
                            <div class="table-responsive" style="border-top-style: none;background: #171921;">
                                <table class="table table-striped table tablesorter" id="ipi-table">
                                    ...
                                    <c:forEach items="${requestScope.pageByPrice.items}" var="book">
                ...
                <div style="width: 100%;height: 50px;">
                    <nav class="pagination" style="background: var(--bs-gray-dark);position: static;">
                        <ul class="pagination">
                            <li class="page-item"><a class="page-link" aria-label="Previous" href="${ requestScope.pageByPrice.url }&pageNo=1"><span aria-hidden="true">首页</span></a></li>
                            <%--大于首页，才显示--%>
                            <c:if test="${requestScope.pageByPrice.pageNo > 1}">
                                <li class="page-item"><a class="page-link" aria-label="Previous" href="${ requestScope.pageByPrice.url }&pageNo=${requestScope.pageByPrice.pageNo-1}"><span aria-hidden="true">上页</span></a></li>
                            </c:if>
                            <%--页码输出的开始--%>
                            <c:choose>
                                <%--情况1：如果总页码小于等于5的情况，页码的范围是：1-总页码--%>
                                <c:when test="${ requestScope.pageByPrice.pageTotal <= 5 }">
                                    <c:set var="begin" value="1"/>
                                    <c:set var="end" value="${requestScope.pageByPrice.pageTotal}"/>
                                </c:when>
                                <%--情况2：总页码大于5的情况--%>
                                <c:when test="${requestScope.pageByPrice.pageTotal > 5}">
                                    <c:choose>
                                        <%--小情况1：当前页码为前面3个：1，2，3的情况，页码范围是：1-5.--%>
                                        <c:when test="${requestScope.pageByPrice.pageNo <= 3}">
                                            <c:set var="begin" value="1"/>
                                            <c:set var="end" value="5"/>
                                        </c:when>
                                        <%--小情况2：当前页码为最后3个，8，9，10，页码范围是：总页码减4 - 总页码--%>
                                        <c:when test="${requestScope.pageByPrice.pageNo > requestScope.pageByPrice.pageTotal-3}">
                                            <c:set var="begin" value="${requestScope.pageByPrice.pageTotal-4}"/>
                                            <c:set var="end" value="${requestScope.pageByPrice.pageTotal}"/>
                                        </c:when>
                                        <%--小情况3：4，5，6，7，页码范围是：当前页码减2 - 当前页码加2--%>
                                        <c:otherwise>
                                            <c:set var="begin" value="${requestScope.pageByPrice.pageNo-2}"/>
                                            <c:set var="end" value="${requestScope.pageByPrice.pageNo+2}"/>
                                        </c:otherwise>
                                    </c:choose>
                                </c:when>
                            </c:choose>
                            ...
                            <c:forEach begin="${begin}" end="${end}" var="i">
                                <c:if test="${i != requestScope.pageByPrice.pageNo}">
                                    <li class="page-item"><a class="page-link" href="${ requestScope.pageByPrice.url }&pageNo=${i}">${i}</a></li>
                                </c:if>
                                <c:if test="${i == requestScope.pageByPrice.pageNo}">
                                    <li class="page-item"><a class="page-link" href="${ requestScope.pageByPrice.url }&pageNo=${i}">${i}</a></li>
                                </c:if>
                            ...
                            <c:if test="${requestScope.pageByPrice.pageNo < requestScope.pageByPrice.pageTotal}">
                                <li class="page-item"><a class="page-link" aria-label="Next" href="${ requestScope.pageByPrice.url }&pageNo=${requestScope.pageByPrice.pageNo+1}"><span aria-hidden="true">下页</span></a></li>
                            </c:if>
                            <li class="page-item"><a class="page-link" aria-label="Next" href="${ requestScope.pageByPrice.url }&pageNo=${requestScope.pageByPrice.pageTotal}"><span aria-hidden="true">末页</span></a></li>
                        </ul>
                    </nav>
                </div>
            </div>
